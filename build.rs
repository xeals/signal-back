...
// println!("cargo:rustc-env=PROTOC_INCLUDE={}",
// protoc_include_dir.display()); println!("cargo:
// rerun-if-env-changed=PROTOC"); println!("cargo:
// rerun-if-env-changed=PROTOC_INCLUDE"); }

// /// Returns `true` if the environment already contains the `PROTOC` and
// `PROTOC_INCLUDE` variables. fn env_contains_protoc() -> bool {
//     let protoc = match env::var("PROTOC") {
//         Ok(val) => PathBuf::from(val),
//         Err(env::VarError::NotPresent) => return false,
// Err(env::VarError::NotUnicode(..)) => panic!("PROTOC environment
// variable is not valid UTF-8"),     };

//     let protoc_include = match env::var("PROTOC_INCLUDE") {
//         Ok(val) => PathBuf::from(val),
// Err(env::VarError::NotPresent) => panic!("PROTOC_INCLUDE environment
// variable not set (PROTOC is set)"), Err(env::VarError::NotUnicode(..
// )) => panic!("PROTOC_INCLUDE environment variable is not valid UTF-8"), };
//

//     if !protoc.exists() {
// panic!("PROTOC environment variable points to non-existent file
// ({:?})", protoc);     }
//     if !protoc_include.exists() {
// panic!("PROTOC_INCLUDE environment variable points to non-existent
// directory ({:?})",                protoc_include);
//     }
